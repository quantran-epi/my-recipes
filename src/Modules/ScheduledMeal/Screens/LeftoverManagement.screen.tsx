import { CalendarOutlined, CheckCircleOutlined, DeleteOutlined, FilterOutlined, FireOutlined, RestOutlined, RollbackOutlined, SearchOutlined } from '@ant-design/icons';
...
const formatStoredAt = (stored?: string): string => {
    if (!stored) return '';
    return `lưu ${dayjs(stored).format('DD/MM HH:mm')}`;
};
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '5px minmax(0, 1fr)', minWidth: 0 }}>
            ...
            <div style={{ padding: '11px 12px', minWidth: 0 }}>
                ...
                {item.status === 'available' && <Stack wrap='wrap' gap={6} style={{ marginTop: 10 }}>
                    <Button icon={<RollbackOutlined />} onClick={onEatOne} style={{ borderRadius: 999 }}>Ăn 1 phần</Button>
                    <Button icon={<CheckCircleOutlined />} onClick={onFinish} style={{ borderRadius: 999 }}>Đã hết</Button>
